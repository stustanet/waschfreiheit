...
static void init(void *drvdata)
{
	...
	/*
	 * NOTE: Doing the initialization here instead of using the initializer
	 *       to init these structs during load is not the most beautiful
	 *       solution but it avoids wasting space in the 'data' section for
	 *       initialization of the xfer buffers.
	 */

#if defined(USE_STM32F4_USBH_DRIVER_FS)
	if (dev == &driver_data_fs)
	{
		dev->base = USB_OTG_FS_BASE;
		dev->channels = channels_fs;
		dev->num_channels = NUM_CHANNELS_FS;
	}
#endif

#if defined(USE_STM32F4_USBH_DRIVER_HS)
	if (dev == &driver_data_hs)
	{
		dev->base = USB_OTG_HS_BASE;
		dev->channels = channels_hs;
		dev->num_channels = NUM_CHANNELS_HS;
	}
#endif
