	...
	while (i < descriptors_len) {
		...
		case USB_DT_INTERFACE:
		{
			...
			if (find_driver(dev, &device_info)) {
				...
				LOG_PRINTF("ifProto: %u\n", device_info.ifaceProtocol);
				LOG_PRINTF("ifSubclass: %u\n", device_info.ifaceSubClass);
				LOG_PRINTF("ifclass: %u\n", device_info.ifaceClass);
				LOG_PRINTF("pid: %u\n", device_info.idProduct);
				LOG_PRINTF("vid: %u\n", device_info.idVendor);
				LOG_PRINTF("devProto: %u\n", device_info.deviceProtocol);
				LOG_PRINTF("devSubclass: %u\n", device_info.deviceSubClass);
				LOG_PRINTF("devClass: %u\n", device_info.deviceClass);
...
static void retransmit_setup_data(usbh_device_t *dev)
{
	dev->control.state = USBH_CONTROL_STATE_SETUP;
	device_xfer_control_write_setup(&dev->control.setup_data, sizeof(dev->control.setup_data), control_state_machine, dev);
}
...
//	LOG_PRINTF("\nSTATE: %d\n", state);
	...
	case USBH_ENUM_STATE_SET_ADDRESS:
		...
		default:
			...
			LOG_PRINTF("XFER ERROR DURING ENUMERATE (SET ADDRESS)-> RETRY");
			retransmit_setup_data(dev);
			break;
		}
		break;
	...
	case USBH_ENUM_STATE_DEVICE_DT_READ_COMPLETE:
		{
			...
			default:
				//device_enumeration_terminate(dev);
				//ERROR(cb_data.status);
				LOG_PRINTF("XFER ERROR DURING ENUMERATE (DT_READ)-> RETRY");
				retransmit_setup_data(dev);
				break;
			}
		}
